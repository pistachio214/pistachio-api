...
import com.pistachio.common.utils.ip.IpUtil;
...
/**
 * @description: 切面处理类，操作日志异常日记记录处理哦
...
public class OperLogAspect {
    ...
    @Value("${pistachio.version}")
    private String operVer;

    @Autowired
    private ISysLogService iSysOperLogService;

    /**
     * 设置操作日志切入点 记录操作日志 在注解的位置切入代码
     */
    @Pointcut("@annotation(com.pistachio.framework.annotation.OperLog)")
    ...
    public void saveOperLog(JoinPoint joinPoint, Object keys) throws IOException {
        ...
        try {
            ...
            // 返回结果
            sysOperLog.setOperRespParam(GsonUtil.toJSONString(keys));
    ...
    public void saveExceptionLog(JoinPoint joinPoint, Throwable e) throws IOException {
        ...
        try {
            ...
            iSysOperLogService.save(sysExceptionLog);
    ...
    public String getParamToString(HttpServletRequest request) throws IOException {
        ...
        switch (request.getMethod()) {
            case "GET":
                ...
                params = GsonUtil.toJSONString(rtnMap);
                break;
            case "POST":
            case "PUT":
